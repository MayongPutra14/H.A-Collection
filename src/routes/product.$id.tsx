...
export const Route = createFileRoute("/product/$id")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: loaderData ? `${loaderData.product.name} — H.A Collection` : "Product" },
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-6 py-24 text-center">
      ...
      <Link to="/catalog" className="mt-6 inline-block text-primary underline">Back to catalog</Link>
    </div>
  ),
...
function ProductPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 pt-8 pb-24">
      <Link to="/catalog" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-8">
        <ArrowLeft className="h-4 w-4" /> {t("nav_catalog")}
      </Link>
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
        <div className="overflow-hidden rounded-xl bg-secondary/40 aspect-[4/5]">
          <img
        ...
        <div className="flex flex-col justify-center">
          <div className="text-xs uppercase tracking-[0.3em] text-muted-foreground mb-3">{product.category}</div>
          ...
          <dl className="mt-8 grid grid-cols-2 gap-6 text-sm border-t border-border pt-6">
            <div>
              <dt className="text-xs uppercase tracking-wider text-muted-foreground mb-1">{t("material")}</dt>
              <dd>{product.material}</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-wider text-muted-foreground mb-1">{t("size")}</dt>
